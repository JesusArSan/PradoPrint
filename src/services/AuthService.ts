...
import { RegisterUserInput, LoginUserInput } from '../types/validation.ts';
...
export class AuthService {
  ...
  async register(data: RegisterUserInput) {
    try {
      const usuario = await prisma.usuario.registra(
        data.email,
        data.nombre,
        data.contraseña
      );
  ...
  async login(data: LoginUserInput) {
    try {
      const usuario = await prisma.usuario.autentifica(
        data.email,
        data.contraseña
      );
